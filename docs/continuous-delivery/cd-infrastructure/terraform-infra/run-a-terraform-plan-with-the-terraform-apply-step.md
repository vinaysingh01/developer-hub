...
You can use an AWS Connector to have the terraform plan and apply step assumes a role to perform the provisioning of infrastructure. It's an optional configuration that takes in AWS Connector, a Region, and a Role ARN. The Terraform step will use these parameters to authenticate with the AWS account targetted for infrastructure provisioning.
